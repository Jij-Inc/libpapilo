...
## 5. Presolver to Reduction Type Mapping

For a detailed mapping of which presolvers generate which `ReductionType` entries, see the "Presolver to Postsolve Reduction Mapping" section in [PRESOLVE.md](PRESOLVE.md).

Key points for postsolve implementation:

- **Minimal implementation**: If you only need primal-only postsolve and can control which presolvers are enabled, you may only need to implement:
  - `kFixedCol` - Simple assignment of fixed value
  - `kSubstitutedCol` / `kSubstitutedColWithDual` - Solve linear equation for substituted variable

- **`kParallelCol` complexity**: This reduction type requires disaggregating a merged variable back into two separate variables with their original bounds. It is exclusively generated by `ParallelColDetection` and can be avoided by disabling `parallelcols.enabled`.

- **Row-only reductions**: Types like `kRedundantRow`, `kRowBoundChange` primarily affect constraint metadata and may not require variable value reconstruction in primal-only postsolve scenarios.
