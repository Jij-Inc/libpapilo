...
TEST_CASE( "presolve-common", "[libpapilo]" )
{
   ...
   {
      // Test Purpose: Verify all presolve utility objects can be created and freed without errors
      ...
      // Test Timer object
      libpapilo_timer_t* timer = libpapilo_timer_create();
   ...
   {
      // Test Purpose: Verify PostsolveStorage can be created with proper dependencies
   ...
   {
      // Test Purpose: Verify ProblemUpdate can be created and basic operations work
      ...
      libpapilo_reductions_t* reductions = libpapilo_problem_update_get_reductions( update );
      REQUIRE( reductions != nullptr );
      REQUIRE( libpapilo_reductions_get_size( reductions ) == 0 );
   ...
   {
      // Test Purpose: Verify SingletonCols presolver can be executed without errors
      // This is a minimal test - comprehensive tests will be in SingletonColsTest.cpp
      ...
      // Force calculation of singleton rows (mimic forceCalculationOfSingletonRows)
      libpapilo_problem_recompute_locks( problem );
      libpapilo_problem_update_trivial_column_presolve( update );
      libpapilo_problem_recompute_activities( problem );
      ...
      // Should return some status (not testing specific behavior here, just that it doesn't crash)
      ...
      {
         libpapilo_reduction_info_t info = libpapilo_reductions_get_info( reductions, i );
         // Just verify the structure is populated (values can be anything valid)
         // No specific assertions on values since this is a unit test, not behavior test
   ...
   SECTION( "name arrays api" )
   {
      ...
      REQUIRE( std::string( libpapilo_problem_get_variable_name( problem, 0 ) ) == "x1" );
      REQUIRE( std::string( libpapilo_problem_get_variable_name( problem, 1 ) ) == "x2" );
      REQUIRE( std::string( libpapilo_problem_get_variable_name( problem, 2 ) ) == "x3" );
      REQUIRE( std::string( libpapilo_problem_get_constraint_name( problem, 0 ) ) == "c1" );
      REQUIRE( std::string( libpapilo_problem_get_constraint_name( problem, 1 ) ) == "c2" );
