...
libpapilo_problem_t* setupProblemWithOnlyOneEntryIn1stRowAndColumn();
libpapilo_problem_t* setupProblemWithSingletonColumn();
libpapilo_problem_t* setupProblemWithSingletonColumnInEquationWithNoImpliedBounds(
    double coefficient, double upper_bound, double lower_bound);
libpapilo_problem_t* setupProblemWithSingletonColumnInEquationWithInfinityBounds();

void forceCalculationOfSingletonRows(libpapilo_problem_t* problem,
...
TEST_CASE("happy-path-singleton-column", "[presolve]")
{
    ...
    libpapilo_singleton_cols_t* presolvingMethod = libpapilo_singleton_cols_create();
    libpapilo_reductions_t* reductions = libpapilo_reductions_create();

    libpapilo_presolve_status_t presolveStatus = 
        libpapilo_singleton_cols_execute(presolvingMethod, problem, problemUpdate, 
                                         num, reductions, timer, &cause);
...
TEST_CASE("happy-path-singleton-column-equation", "[presolve]")
{
    ...
    libpapilo_timer_t* timer = libpapilo_timer_create();
    libpapilo_message_t* msg = libpapilo_message_create();
    libpapilo_problem_t* problem = setupProblemWithOnlyOneEntryIn1stRowAndColumn();
    libpapilo_statistics_t* statistics = libpapilo_statistics_create();
    libpapilo_presolve_options_t* presolveOptions = libpapilo_presolve_options_create();
    libpapilo_postsolve_storage_t* postsolve = 
        libpapilo_postsolve_storage_create(problem, num, presolveOptions);
    ...
    libpapilo_reduction_info_t reduction5 = libpapilo_reductions_get_info(reductions, 5);
    REQUIRE(reduction5.col == LIBPAPILO_ROW_REDUCTION_RHS);
    REQUIRE(reduction5.row == 1);
    REQUIRE(reduction5.newval == 2.5);

    libpapilo_reduction_info_t reduction6 = libpapilo_reductions_get_info(reductions, 6);
    REQUIRE(reduction6.col == 0);
    REQUIRE(reduction6.row == 1);
    REQUIRE(reduction6.newval == 0.75);

    libpapilo_reduction_info_t reduction7 = libpapilo_reductions_get_info(reductions, 7);
...
TEST_CASE("happy-path-singleton-column-implied-bounds-negative-coeff-pos-bounds", "[presolve]")
{
    ...
    libpapilo_problem_t* problem = 
        setupProblemWithSingletonColumnInEquationWithNoImpliedBounds(-1.0, 10.0, 3.0);
    libpapilo_statistics_t* statistics = libpapilo_statistics_create();
    libpapilo_presolve_options_t* presolveOptions = libpapilo_presolve_options_create();
    libpapilo_postsolve_storage_t* postsolve = 
        libpapilo_postsolve_storage_create(problem, num, presolveOptions);
...
TEST_CASE("happy-path-singleton-column-implied-bounds-negative-coeff-neg-bounds", "[presolve]")
{
    ...
    libpapilo_num_t* num = libpapilo_num_create();
    libpapilo_problem_t* problem = 
        setupProblemWithSingletonColumnInEquationWithNoImpliedBounds(-1.0, -3.0, -10.0);
    libpapilo_statistics_t* statistics = libpapilo_statistics_create();
    libpapilo_presolve_options_t* presolveOptions = libpapilo_presolve_options_create();
    libpapilo_postsolve_storage_t* postsolve = 
        libpapilo_postsolve_storage_create(problem, num, presolveOptions);
...
libpapilo_problem_t* setupProblemWithOnlyOneEntryIn1stRowAndColumn()
{
    ...
    libpapilo_problem_builder_set_problem_name(pb, "singleton column & row matrix with equation");
