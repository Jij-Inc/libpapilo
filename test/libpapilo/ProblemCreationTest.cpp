...
TEST_CASE( "problem-creation", "[libpapilo]" )
{
   ...
   SECTION( "complete workflow - construction and data retrieval" )
   {
      // Create a simple LP problem:
      // minimize: 2*x + 3*y + 1.5
      // subject to: x + y >= 1
      //            2*x + y <= 3
      //            x in [-1, 5], y >= 0
      ...
      // Set variable bounds: x in [-1, 5], y >= 0
      double lb[] = { -1.0, 0.0 };
      ...
      result =
          papilo_get_objective( papilo, retrieved_coeffs, &retrieved_offset );
      REQUIRE( result == PAPILO_OK );
      ...
      result =
          papilo_get_row_bounds_all( papilo, retrieved_lhs, retrieved_rhs );
      REQUIRE( result == PAPILO_OK );
      ...
      result = papilo_get_matrix( papilo, retrieved_rows, retrieved_cols,
                                  retrieved_values );
      REQUIRE( result == PAPILO_OK );
      ...
      for( int i = 0; i < 4; ++i )
      {
         retrieved.emplace_back( retrieved_rows[i], retrieved_cols[i],
                                 retrieved_values[i] );
      }
      ...
      for( int i = 0; i < 4; ++i )
      {
         ...
         REQUIRE( std::get<2>( retrieved[i] ) ==
                  Catch::Approx( std::get<2>( expected[i] ) ) );
      }

      papilo_free( papilo );
   }

   SECTION( "parameter validation and error handling" )
   {
      papilo_t* papilo = papilo_create();
      REQUIRE( papilo != nullptr );
      ...
      // === INDIVIDUAL BOUND SETTING WITH VALIDATION ===

      // Set valid dimensions first
      result = papilo_set_problem_dimensions( papilo, 2, 3, 6 );
      REQUIRE( result == PAPILO_OK );

      // Test individual column bounds (valid cases)
      ...
      // Test individual row bounds (valid cases)
   ...
   SECTION( "state management and lifecycle" )
   {
      papilo_t* papilo = papilo_create();
      REQUIRE( papilo != nullptr );

      // Set up a minimal valid problem
      int result = papilo_set_problem_dimensions( papilo, 2, 2, 4 );
      REQUIRE( result == PAPILO_OK );
      ...
      // === POST-BUILD MODIFICATION ATTEMPTS (should fail) ===

      // Try to modify objective after building
      result = papilo_set_objective( papilo, obj, 0.0 );
      REQUIRE( result == PAPILO_ERROR_INVALID_STATE );

      // Try to add entry after building
      result = papilo_add_entry( papilo, 0, 0, 1.0 );
      REQUIRE( result == PAPILO_ERROR_INVALID_STATE );

      // Try to set dimensions after building
      result = papilo_set_problem_dimensions( papilo, 3, 3, 9 );
      REQUIRE( result == PAPILO_ERROR_INVALID_STATE );

      // Try to set bounds after building
      result = papilo_set_col_bounds( papilo, 0, -1.0, 1.0 );
      REQUIRE( result == PAPILO_ERROR_INVALID_STATE );

      result = papilo_set_row_bounds( papilo, 0, -1.0, 1.0 );
      REQUIRE( result == PAPILO_ERROR_INVALID_STATE );
