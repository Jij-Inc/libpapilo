...
class PresolveMethod
{
 ...
 protected:
   ...
   static bool
   is_interrupted( const Timer& timer, double tlim, std::function<bool()> early_exit_callback = nullptr )
   {
      return is_time_exceeded( timer, tlim ) ||
             is_user_interrupted( early_exit_callback );
   }
