...
## Presolver to Postsolve Reduction Mapping

This section documents which presolvers generate which `ReductionType` entries in the `PostsolveStorage`. Understanding this mapping is essential for implementations that need to support specific presolvers.

### Reduction Methods and Their Postsolve Types

The following table shows the mapping from `Reductions` class methods (called by presolvers) to the `ReductionType` stored in `PostsolveStorage`:

| Reductions Method | ReductionType | Description |
|-------------------|---------------|-------------|
| `fixCol` | `kFixedCol` | Variable fixed to a constant value |
| `fixColPositiveInfinity` | `kFixedInfCol` | Variable fixed at positive infinity |
| `fixColNegativeInfinity` | `kFixedInfCol` | Variable fixed at negative infinity |
| `changeColLB` / `changeColUB` | `kVarBoundChange` | Variable bound tightened |
| `aggregateFreeCol` | `kSubstitutedCol` or `kSubstitutedColWithDual` | Variable substituted via equality |
| `substituteColInObjective` | `kSubstitutedCol` or `kSubstitutedColWithDual` | Variable substituted in objective |
| `replaceCol` | `kSubstitutedCol` | Variable replaced with expression |
| `mark_parallel_cols` | `kParallelCol` | Parallel columns merged |
| `markRowRedundant` | `kRedundantRow` | Row marked as redundant |
| `changeRowLHS` / `changeRowRHS` | `kRowBoundChange` | Row bound changed |
| `sparsify` | `kCoefficientChange` | Matrix coefficients modified |
| `impliedInteger` | *(no postsolve)* | Variable marked as implied integer |

### Presolvers and Generated Reduction Types

The following table shows which presolvers call which reduction methods:

| Presolver | Timing | Primary Reduction Methods |
|-----------|--------|---------------------------|
| **SingletonCols** | Fast | `fixCol`, `substituteColInObjective`, `markRowRedundant`, `changeRowLHS/RHS` |
| **CoefficientStrengthening** | Fast | `changeRowLHS/RHS` |
| **ConstraintPropagation** | Fast | `fixCol`, `changeColLB/UB`, `markRowRedundant` |
| **FixContinuous** | Medium | `fixCol` |
| **SimpleProbing** | Medium | `replaceCol` |
| **ParallelRowDetection** | Medium | `markRowRedundant` |
| **ParallelColDetection** | Medium | `mark_parallel_cols` |
| **SingletonStuffing** | Medium | `fixCol`, `substituteColInObjective`, `markRowRedundant`, `changeRowLHS/RHS` |
| **DualFix** | Medium | `fixCol`, `fixColPositiveInfinity`, `fixColNegativeInfinity`, `changeColLB/UB` |
| **SimplifyInequalities** | Medium | `changeRowLHS/RHS` |
| **SimpleSubstitution** | Medium | `aggregateFreeCol`, `changeColLB/UB` |
| **CliqueMerging** | Medium | `markRowRedundant` |
| **ImplIntDetection** | Exhaustive | `impliedInteger` |
| **DominatedCols** | Exhaustive | `fixCol` |
| **DualInfer** | Exhaustive | `fixCol`, `changeRowLHS/RHS` |
| **Probing** | Exhaustive | `changeColLB/UB`, `replaceCol` |
| **Substitution** | Exhaustive | `aggregateFreeCol` |
| **Sparsify** | Exhaustive | `sparsify` |

### Key Observations

1. **`kParallelCol` is exclusively generated by `ParallelColDetection`**: If you need to avoid this reduction type, disable `parallelcols.enabled`.

2. **`kFixedCol` is widely generated**: Many presolvers can fix variables, including `SingletonCols`, `ConstraintPropagation`, `DualFix`, `DominatedCols`, etc.

3. **`kSubstitutedCol` / `kSubstitutedColWithDual` come from substitution operations**: Generated by `SingletonCols`, `SingletonStuffing`, `SimpleSubstitution`, `SimpleProbing`, `Probing`, and `Substitution`.

4. **Row-only reductions**: Some reductions like `kRedundantRow`, `kRowBoundChange` only affect row information and may not require complex variable value reconstruction during postsolve.

5. **Primal vs Full postsolve**: In `PostsolveType::kPrimal` mode, dual-related information is not stored, which simplifies postsolve implementation but limits the ability to recover dual solutions.

### Parameter Keys for Disabling Presolvers

Each presolver can be disabled using the parameter key `{presolver_name}.enabled`. For example:

- `parallelcols.enabled` - Disable parallel column detection
- `parallelrows.enabled` - Disable parallel row detection
- `dualfix.enabled` - Disable dual fixing
- `probing.enabled` - Disable probing
- `substitution.enabled` - Disable free variable substitution
- `sparsify.enabled` - Disable sparsification

See the C API function `libpapilo_presolve_set_param_bool()` for programmatic control.
